  ...
  async function isThrottled(req) {
    for (let i = 0, max = throttles.length; i < max; i++) {
      ...
      if (matched === true) {
        ...
        const currentStep = Math.ceil(currentTimestamp / periodMs) * periodMs
        const prevStep = currentStep - periodMs

        if (key !== undefined && limit !== undefined && period !== undefined) {
          const currentCount = await store.increment(`${key}-${currentStep}`, period)
          const prevCount = await store.get(`${key}-${prevStep}`, period)

          if (currentCount >= limit || prevCount >= limit) {
            return true
          }
        }
      }
